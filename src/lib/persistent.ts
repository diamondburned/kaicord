...
type StoresValues<S extends Stores, T> = Parameters<Parameters<typeof store.derived<S, T>>[1]>[0];

export function derived<S extends Stores, T>(
  stores: S,
  key: string,
  def: T,
  start: (values: StoresValues<S, T>, set: (value: T) => void) => store.Unsubscriber | void
) {
  const v = localStorage.getItem(key);
  if (v) {
    try {
